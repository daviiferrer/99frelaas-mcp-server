...
test("parsers", async () => {
  ...
  const listHtml = `
    <li class="result-item">
      <a href="/project/migrar-site-744895">Migrar</a>
      <a href="/user/joao-cliente">João Cliente</a>
      <p class="item-text information">Resumo <b>curto</b></p>
      <div>Publicado: 2 horas atrás</div>
      <div>Tempo restante: 29 dias e 22 horas</div>
      <div>Propostas: 10</div>
      <div>Interessados: 11</div>
      <div>Nível de experiência: Iniciante</div>
      <div>Subcategoria: Desenvolvimento Web</div>
    </li>
  `;
  ...
  assert.equal(items[0].publishedText, "2 horas atrás");
  assert.equal(items[0].remainingText, "29 dias e 22 horas");
  assert.equal(items[0].proposalsCount, 10);
  assert.equal(items[0].interestedCount, 11);
  assert.equal(items[0].experienceLevel, "Iniciante");
  assert.equal(items[0].subcategoryName, "Desenvolvimento Web");
  assert.equal(items[0].client.username, "joao-cliente");

  const detailHtml = `
    <title>Portal Web Financeiro (MVP) | 99Freelas</title>
    <h2>DescriÃƒÂ§ÃƒÂ£o do Projeto</h2><p>Desc principal</p><h2>Outro</h2>
    Valor MÃƒÂ­nimo: <b>R$ 50,00</b>
    Valor MÃƒÂ¡ximo: <b>R$ 100,00</b>
    Visibilidade: <b>Aberto</b>
    Categoria: Web, Mobile & Software
    Subcategoria: Outra - Web, Mobile & Software
    Nível de experiência: Iniciante
    Publicado: ontem às 23:58
    Tempo restante: 29 dias e 22 horas
    Propostas: 10
    Interessados: 10
    Orçamento: Aberto
    <li class="proposal-item"><a href="/user/joao">Joao</a> Submetido: 1 dia atrÃƒÂ¡s Premium</li>
    <div class="project-description"><ul><li>Login individual</li><li>Dashboard com gráficos</li></ul></div>
    React Next.js Node.js Python PostgreSQL MySQL
    ...
    (5.0 - 1 avaliaÃ§Ã£o)
    Projetos concluÃ­dos: 1 | RecomendaÃ§Ãµes: 2 | Registrado desde: 24/03/2026
  `;
  ...
  assert.equal(detail.title, "Portal Web Financeiro (MVP)");
  assert.equal(detail.categoryName, "Web, Mobile & Software");
  assert.equal(detail.subcategoryName, "Outra - Web, Mobile & Software");
  assert.equal(detail.experienceLevel, "Iniciante");
  assert.equal(detail.publishedText, "ontem às 23:58");
  assert.equal(detail.remainingText, "29 dias e 22 horas");
  assert.equal(detail.proposalsCount, 10);
  assert.equal(detail.interestedCount, 10);
  assert.equal(detail.budgetText, "Aberto");
  assert.equal(detail.featureList[0], "Login individual");
  assert.ok(detail.preferredTechnologies.includes("React"));
  ...
  assert.equal(detail.competitors[0].profileUrl, "https://www.99freelas.com.br/user/joao");
  ...
  const profileHtml = `
    <h1>FÃ¡bio p. | Contratante | 99Freelas</h1>
    <p>(5.0 - 1 avaliaÃ§Ã£o)</p>
    <p>Projetos concluÃ­dos: 1 | RecomendaÃ§Ãµes: 1 | Registrado desde: 24/03/2026</p>
    <h2>HistÃ³rico de projetos & AvaliaÃ§Ãµes:</h2>
    <a href="/project/clipador-profissional-para-reels-tiktok-e-shorts-com-foco-em-viralizacao-matheus-b-739916">Clipador profissional para reels, TikTok e shorts com foco em viralizaÃ§Ã£o - Matheus B.</a>
    <p>"Ã“timo cliente! Foi claro em todas as informaÃ§Ãµes!"</p>
    <p>5.0</p>
    <p>mar. 2026 - mar. 2026</p>
    <h2>Projetos (Aguardando Propostas):</h2>
    <a href="/project/gestao-de-conteudo-e-crescimento-digital-para-empresa-744497">GestÃ£o de conteÃºdo e crescimento digital para empresa</a>
